...
void main(void)
{
	...
	float x = sin((inverse(camMatrix) * vec4(exSurface, 1.0)).x);
	if (x < 0) out_Color = vec4(res, 1.0f) * texture(texUnit0, texCoord);
	else out_Color = vec4(res, 1.0f) * texture(texUnit1, texCoord);
}
